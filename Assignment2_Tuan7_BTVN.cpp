...
void find_years_with_same_data(struct Population population[], int size) {
    printf("cac nam co cung so nguoi sinh ra:\n");
    for (int i = 0; i < size; i++) {
        for (int j = i + 1; j < size; j++) {
            if (population[i].data == population[j].data) {
                printf("{%d, %d} va {%d, %d}\n", population[i].year, population[i].data, population[j].year, population[j].data);
            }
        }
    }
}
...
void find_years_min_max_data(struct Population population[], int size) {
    ...
    // In ra kqua max
    printf("cac nam co so nguoi max (%d nguoi):\n", max_data);
    for (int i = 0; i < size; i++) {
        if (population[i].data == max_data) {
            printf("{%d, %d}\n", population[i].year, population[i].data);
        }
    }
    // In ra kqua min
    printf("cac nam co so nguoi min (%d nguoi):\n", min_data);
    for (int i = 0; i < size; i++) {
        if (population[i].data == min_data) {
            printf("{%d, %d}\n", population[i].year, population[i].data);
        }
